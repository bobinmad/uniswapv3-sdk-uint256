...
func TestNewPool(t *testing.T) {
	_, err := NewPool(USDC, entities.WETH9[3], constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
	assert.ErrorIs(t, err, entities.ErrDifferentChain, "cannot be used for tokens on different chains")

	_, err = NewPool(USDC, entities.WETH9[1], 1e6, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
	assert.ErrorIs(t, err, ErrFeeTooHigh, "fee cannot be more than 1e6'")

	_, err = NewPool(USDC, USDC, constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
	assert.ErrorIs(t, err, entities.ErrSameAddress, "cannot be used for the same token")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 1, nil)
	assert.ErrorIs(t, err, ErrInvalidSqrtRatioX96, "price must be within tick price bounds")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeMedium, new(big.Int).Add(utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(1)), big.NewInt(0), -1, nil)
	assert.ErrorIs(t, err, ErrInvalidSqrtRatioX96, "price must be within tick price bounds")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
	assert.NoError(t, err, "works with valid arguments for empty pool medium fee")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeLow, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
	assert.NoError(t, err, "works with valid arguments for empty pool low fee")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeHigh, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
...
func TestToken0(t *testing.T) {
	pool, _ := NewPool(USDC, DAI, constants.FeeLow, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
	assert.Equal(t, pool.Token0, DAI, "always is the token that sorts before")

	pool, _ = NewPool(DAI, USDC, constants.FeeLow, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
	assert.Equal(t, pool.Token0, DAI, "always is the token that sorts before")
}
...
func TestChainID(t *testing.T) {
	pool0, _ := NewPool(USDC, DAI, constants.FeeLow, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
	assert.Equal(t, pool0.ChainID(), uint(1), "returns the token0 chainId")

	pool1, _ := NewPool(DAI, USDC, constants.FeeLow, utils.EncodeSqrtRatioX96(constants.One, constants.One).ToBig(), big.NewInt(0), 0, nil)
