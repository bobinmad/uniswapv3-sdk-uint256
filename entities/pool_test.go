...
func TestNewPool(t *testing.T) {
	_, err := NewPool(USDC, entities.WETH9[3], constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
	assert.ErrorIs(t, err, entities.ErrDifferentChain, "cannot be used for tokens on different chains")

	_, err = NewPool(USDC, entities.WETH9[1], 1e6, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
	assert.ErrorIs(t, err, ErrFeeTooHigh, "fee cannot be more than 1e6'")

	_, err = NewPool(USDC, USDC, constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
	assert.ErrorIs(t, err, entities.ErrSameAddress, "cannot be used for the same token")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 1, nil)
	assert.ErrorIs(t, err, ErrInvalidSqrtRatioX96, "price must be within tick price bounds")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeMedium, new(big.Int).Add(utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(1)), big.NewInt(0), -1, nil)
	assert.ErrorIs(t, err, ErrInvalidSqrtRatioX96, "price must be within tick price bounds")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
	assert.NoError(t, err, "works with valid arguments for empty pool medium fee")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeLow, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
	assert.NoError(t, err, "works with valid arguments for empty pool low fee")

	_, err = NewPool(USDC, entities.WETH9[1], constants.FeeHigh, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
...
func TestToken0(t *testing.T) {
	pool, _ := NewPool(USDC, DAI, constants.FeeLow, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
	assert.Equal(t, pool.Token0, DAI, "always is the token that sorts before")

	pool, _ = NewPool(DAI, USDC, constants.FeeLow, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
	assert.Equal(t, pool.Token0, DAI, "always is the token that sorts before")
}
...
func TestToken0Price(t *testing.T) {
	a1 := new(uint256.Int).Mul(uint256.NewInt(101), uint256.NewInt(1e6))
	a2 := new(uint256.Int).Mul(uint256.NewInt(100), uint256.NewInt(1e18))
	r, _ := utils.GetTickAtSqrtRatio(utils.EncodeSqrtRatioX96(a1, a2).ToBig())
	pool0, _ := NewPool(USDC, DAI, constants.FeeLow, utils.EncodeSqrtRatioX96(a1, a2).ToBig(), big.NewInt(0), r, nil)
	assert.Equal(t, pool0.Token0Price().ToSignificant(5), "1.01", "returns price of token0 in terms of token1")

	pool1, _ := NewPool(DAI, USDC, constants.FeeLow, utils.EncodeSqrtRatioX96(a1, a2).ToBig(), big.NewInt(0), r, nil)
...
func TestChainID(t *testing.T) {
	pool0, _ := NewPool(USDC, DAI, constants.FeeLow, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
	assert.Equal(t, pool0.ChainID(), uint(1), "returns the token0 chainId")

	pool1, _ := NewPool(DAI, USDC, constants.FeeLow, utils.EncodeSqrtRatioX96(constants.OneU256, constants.OneU256).ToBig(), big.NewInt(0), 0, nil)
